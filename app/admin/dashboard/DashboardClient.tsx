...
export function DashboardClient({ initialAssets }: { initialAssets: AssetMap }) {
  ...
  async function handleUpload(category: AssetCategory, index: number, file: File) {
    const key = slotKey(category, index);
    setStatus(key, { state: "uploading" });

    try {
      const ext = file.name.split(".").pop() ?? "bin";
      const pathname = `${category}-${index}.${ext}`;

      // Browser uploads directly to Vercel Blob — no serverless size limit
      const blob = await uploadToBlob(pathname, file, {
        access: "public",
        handleUploadUrl: "/api/upload",
        clientPayload: JSON.stringify({ category, index: String(index) }),
      });

      setAssets((prev) => {
        const updated = { ...prev };
        const arr = [...updated[category]] as (string | null)[];
        arr[index - 1] = blob.url;
  ...
  async function handleDelete(category: AssetCategory, index: number) {
    ...
    const assetKeyStr = urlPath.split("/").pop() ?? "";
    ...
    try {
      const res = await fetch("/api/delete-asset", {
        ...
        body: JSON.stringify({ key: assetKeyStr }),
      });
      ...
      <div className="max-w-6xl mx-auto px-6 pt-10 flex flex-col gap-10">
        <div>
          ...
          <p className="text-gray-500 text-sm">
            Cliquez sur un slot pour uploader. Les changements sont visibles sur le site en moins d&apos;une minute.
          </p>
        </div>
                    ...
                    <div key={key} className="flex flex-col gap-2">
                      ...
                      <div
                        ...
                        style={{
                          ...
                          border: currentUrl ? "1px solid rgba(124,58,237,0.5)" : "2px dashed rgba(124,58,237,0.3)",
                          transition: "border-color 0.2s",
                        }}
                        onClick={() => { if (status.state !== "uploading") fileInputRefs.current[key]?.click(); }}
                      >
                        {/* Current media */}
                        {currentUrl && status.state !== "uploading" && (
                          isVideo(currentUrl) ? (
                            <video src={currentUrl} autoPlay muted loop playsInline
                              className="absolute inset-0 w-full h-full object-cover" />
                          ) : (
                            <Image src={currentUrl} alt={`${section.label} ${index}`}
                        ...
                        {status.state === "uploading" && (
                          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2"
                            style={{ background: "rgba(13,11,43,0.9)" }}>
                        ...
                        {!currentUrl && status.state === "idle" && (
                          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 group-hover:opacity-80 transition-opacity">
                            ...
                            <span className="text-gray-500 text-xs text-center px-2">Cliquer pour<br />uploader</span>
                          </div>
                        )}
                        {currentUrl && status.state === "idle" && (
                      ...
                      {currentUrl && status.state === "idle" && (
                        <button
                          onClick={(e) => {
                            e.stopPropagation();
                            if (confirm(`Supprimer le slot ${index}?`)) handleDelete(section.category, index);
                          }}
        ...
        <div className="rounded-2xl p-5 flex gap-4 items-start"
          ...
          <div className="flex flex-col gap-1">
            ...
            <ul className="text-gray-400 text-xs flex flex-col gap-1">
              <li>• Cliquez sur un slot vide pour uploader un fichier</li>
              <li>• Cliquez sur un média existant pour le remplacer</li>
